...
def levels(observation, settings, *, side=None):
    ...
    for raw in chain(observation.structural_support_levels, observation.structural_resistance_levels):
        try:
            ...
            v5 = row.get('book_version') in ('causal-swing-closing-book-5','causal-swing-closing-book-6')
            ...
            if (row.get('book_version') not in ('causal-swing-closing-book-4','causal-swing-closing-book-5','causal-swing-closing-book-6') or (row.get('lifecycle') != 'active' and not retained)
