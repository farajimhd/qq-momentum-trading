...
def select_areas(levels, now, *, maximum_width_bps=100., minimum_score=30.):
    # Bound the ENTIRE area, not each adjacent gap: prevents transitive chaining.
    candidates = sorted((r for r in levels if r['side'] == 'resistance'
    ...
    for row in candidates:
        if not all(isfinite(row[k]) for k in ('price', 'lower', 'upper')):
            raise ValueError('Invalid resistance geometry')
    ...
    for group in groups:
        ...
        result.append(dict(id=sha256('|'.join(ids).encode()).hexdigest()[:16],
            ...
            score=round(score, 1), selected=score >= minimum_score, members=ids,
            reasons=[f'{len(ids)} candidates; strongest evidence used, not summed',
                     ...
                     'new resistance role awaiting validation' if representative.get('last_role_change_at') and not representative.get('role_retests') else 'resistance evidence available']))
    return result
