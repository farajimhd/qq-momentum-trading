...
def source_metadata(ticker,session,query=_query):
    ...
    clocks = query(f'SELECT * FROM q_live.historical_event_execution_clock_coverage_v1 FINAL WHERE {predicate}')
...
def bar_sql(ticker,session,left,right,rules):
    ...
    FROM merge('market_sip_compact','^events_({years})$') AS e
    INNER JOIN (SELECT ordinal,execution_timestamp_us FROM q_live.historical_event_execution_clock_v1 FINAL
    ...
    WHERE e.ticker='{ticker}' AND e.sip_timestamp_us>={start_us} AND e.sip_timestamp_us<{end_us}
      ...
      AND (c.execution_timestamp_us=0 OR intDiv(c.execution_timestamp_us,1000000)>=intDiv(e.sip_timestamp_us,1000000))
...
def read_session(ticker,session,client=None):
    ...
    revision,rules = source_metadata(ticker,session,query)
    start,end = session_bounds(session)
    def read(i):
        left,right = start+timedelta(hours=i*2),start+timedelta(hours=(i+1)*2)
        result = query(bar_sql(ticker,session,left,right,rules))
    ...
    if source_metadata(ticker,session,query)[0]['token']!=revision['token']:
