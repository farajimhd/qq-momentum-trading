...
def builds():
    ...
    for path in sorted([*ROOT.glob('*/report.json'), *ROOT.glob('*/*/report.json')]):
        ...
        if report.get('version') not in (VERSION, 'causal-swing-closing-book-1', 'causal-swing-closing-book-2', 'causal-swing-closing-book-3') or proof.get('status') != 'passed':
            continue
...
class BookCursor:
    def __new__(cls, build_id, ticker, fingerprint=None):
        if resolve(build_id).get('version', VERSION) in ('causal-swing-closing-book-1', 'causal-swing-closing-book-2', 'causal-swing-closing-book-3'):
