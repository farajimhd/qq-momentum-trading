...
from scripts import build_swing_structure_book as builder
...
def test_writes_use_fresh_connections_and_expired_reads_are_evicted(tmp_path):
    ports=[]
    class Handler(BaseHTTPRequestHandler):
        protocol_version='HTTP/1.1'
        def do_POST(self):
            body=self.rfile.read(int(self.headers['Content-Length']))
            ports.append(self.client_address)
            response=b'{"ok":1}\n' if body.startswith(b'SELECT') else b''
    ...
    client=storage.Client(env)
    try:
        client.query('SELECT 1','read')
        client.query('INSERT INTO test VALUES (1)','write',read=False)
        client.query('INSERT INTO test VALUES (2)','write',read=False)
        assert len(set(ports))==3
        cached,_=client._pool.get_nowait()
        client._pool.put_nowait((cached,0))
        client.query('SELECT 1','expired_read')
        assert ports[-1] not in ports[:-1] and cached.sock is None
    finally:
        client.close();server.shutdown();server.server_close();thread.join()


def checkpoint_rows():
    return [dict(ticker='TEST',level_id=i,valid_from_us=10,valid_to_us=None,
                 state_json=json.dumps({'id':i}),revision=1) for i in range(3)]


class UncertainStore:
    def __init__(self,accepted=0,conflict=False,always_fail=False):
        self.saved=[];self.attempts=[];self.accepted=accepted;self.conflict=conflict
        self.always_fail=always_fail;self.settled=False
    def settle_write(self,error):
        self.settled=True
    def query(self,sql,label,read=True):
        if read:
            assert self.settled
            return self.saved
        rows=[json.loads(line) for line in sql.splitlines()[1:]]
        self.attempts.append(rows)
        if len(self.attempts)==1 or self.always_fail:
            self.saved.extend(rows[:self.accepted])
            if self.conflict:self.saved[0]['state_json']='conflicting content'
            raise http.client.RemoteDisconnected('Remote end closed connection without response')
        self.saved.extend(rows)


@pytest.mark.parametrize('accepted',[0,1,3])
def test_checkpoint_recovery_retries_only_missing_rows(accepted,monkeypatch):
    monkeypatch.setattr(builder.time,'sleep',lambda _:None)
    client=UncertainStore(accepted)
    rows=checkpoint_rows()
    builder.insert(client,'test','book',rows)
    assert client.saved==rows
    assert len(client.attempts)==(1 if accepted==3 else 2)
    if accepted<3:assert client.attempts[1]==rows[accepted:]


def test_checkpoint_recovery_never_overwrites_conflicting_equal_revision():
    client=UncertainStore(1,conflict=True)
    with pytest.raises(ValueError,match='Conflicting'):
        builder.insert(client,'test','book',checkpoint_rows())
    assert len(client.attempts)==1


def test_checkpoint_recovery_is_bounded(monkeypatch):
    monkeypatch.setattr(builder.time,'sleep',lambda _:None)
    client=UncertainStore(always_fail=True)
    with pytest.raises(http.client.RemoteDisconnected):
        builder.insert(client,'test','book',checkpoint_rows())
    assert len(client.attempts)==3 and not client.saved


def test_lost_session_marker_response_is_verified_without_reinsert():
    client=UncertainStore(1)
    rows=[dict(ticker='TEST',session_date='2026-09-10',closed_at=10.,sequence=3,
               close=10.,state_hash='expected',source_revision='source',revision=1)]
    builder.insert(client,'test','sessions',rows)
    assert client.saved==rows and len(client.attempts)==1


def test_failed_write_carries_identity_for_settlement(tmp_path,monkeypatch):
    env=tmp_path/'test.env';env.write_text('CLICKHOUSE_URL=http://127.0.0.1:1')
    client=storage.Client(env)
    def disconnected(*args):raise http.client.RemoteDisconnected('lost response')
    monkeypatch.setattr(client,'_response',disconnected)
    with pytest.raises(http.client.RemoteDisconnected) as caught:
        client.query('INSERT INTO test VALUES (1)','write',read=False)
    assert caught.value.query_id==client.profiles[-1]['query_id']
    commands=[]
    monkeypatch.setattr(client,'query',lambda *args,**kwargs:commands.append((args,kwargs)) or [])
    client.settle_write(caught.value)
    assert caught.value.query_id in commands[0][0][0]
    assert commands[0][0][0].endswith(' SYNC FORMAT JSONEachRow')
    monkeypatch.setattr(client,'query',lambda *args,**kwargs:[{'kill_status':'cant_cancel'}])
    with pytest.raises(RuntimeError,match='still active'):client.settle_write(caught.value)
    with pytest.raises(ValueError):client.settle_write(ConnectionResetError())
...
@pytest.mark.parametrize('failure_spacing',[0,200])
def test_resume_applies_workers_and_stops_dispatch_after_transport_failures(tmp_path,monkeypatch,failure_spacing):
    ...
    campaign.save(tmp_path,m);spawned=[];clock=[0.]
    class Process:
        ...
        def poll(self):clock[0]+=failure_spacing;return 1
    ...
    assert len(spawned)==(4 if not failure_spacing else 10)
    assert (tmp_path/'STOP').exists()==(failure_spacing==0)
    ...
    assert sum(r['status']=='queued' for r in saved['rows'])==(6 if not failure_spacing else 0)
