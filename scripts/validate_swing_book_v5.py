...
def run(book,session,end_time=None):
    b=resolve(book);source=b['source_book']
    contract=b.get('selection_contract',LEGACY_CONTRACT)
    ...
    for offset in range(0,len(dates),10):
        ...
        for stamp in stamps:
            expected={prefix+a['id']:a for side,prefix in ([('support','s:'),('resistance','r:')] if contract==CONTRACT else [('resistance','r:')])
                      for a in select_areas([json.loads(r['state_json']) for r in fetched if r['valid_from_us']==stamp],stamp/1e6,side=side,role_safe=contract==CONTRACT) if a['selected']}
            actual=rows(f"SELECT * FROM {book}.book FINAL WHERE {'1' if contract==CONTRACT else 'side=-1'} AND valid_from_us<={stamp} AND (valid_to_us IS NULL OR valid_to_us>{stamp})")
            assert set(expected)=={r['level_id'] for r in actual},f'Membership mismatch at {stamp}'
            for r in actual:
                a=expected[r['level_id']]
    ...
    engine=StreamingSwingBookV5(seed,opening.timestamp(),factor,contract=contract)
    durations=[];max_levels=0
    from src.backend.swing_book_source import NY
    end=datetime.fromisoformat(session+'T'+(end_time or '20:00:00')).replace(tzinfo=NY).timestamp()
    for bar in bars:
        if bar[0]>end:break
    ...
    return dict(ticker=b['ticker'],book=book,closing_states_checked=checked,session=session,
        ...
        maximum_visible_levels=max_levels,selection_contract=contract,selection_rebuilds=engine.selection_rebuilds,status='passed')
...
def main():
    ...
    p.add_argument('--end-time',help='Optional validation cutoff in New York time, HH:MM:SS; default session close')
    a=p.parse_args()
    ...
    for book in a.books:
        ...
        results.append(run(book,a.session,a.end_time));a.output.write_text(json.dumps(results,indent=2))
