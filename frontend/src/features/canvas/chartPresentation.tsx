...
function pushUnifiedStructureLevels(
  ...
  segments.forEach(({ end, latest, level, start }) => {
    ...
    if (["clickhouse-closing-book-1", "causal-swing-closing-book-1", "causal-swing-closing-book-2"].includes(String(level.book_version))) {
      const support = level.side > 0;
      const bookLabel = ["causal-swing-closing-book-1", "causal-swing-closing-book-2"].includes(String(level.book_version)) ? "Swing level book" : "Experimental ClickHouse level book";
...
function isQmdUnifiedStructureLevel(value: unknown): value is QmdUnifiedStructureLevel {
  ...
  return (Number.isFinite(Number(row.unified_level_id)) || (["merged-point-minmax-v1", "merged-point-minmax-v2", "merged-point-minmax-v3", "merged-point-minmax-v4"].includes(String(row.load_contract)) && /^merged:[a-f0-9]{24}$/.test(String(row.unified_level_id))))
    ...
    && (["clickhouse-closing-book-1", "causal-swing-closing-book-1", "causal-swing-closing-book-2"].includes(String(row.book_version)) ? Number.isFinite(row.prominence) : Number.isFinite(Number(row.hold_probability)))
