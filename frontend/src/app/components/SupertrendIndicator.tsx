...
export function useSupertrend(storageKey:string,timeframe:string,candles:SupertrendCandle[],asOf?:string) {
  ...
  return {enabled,points:result.points,
    ...
    controls:<>{enabled && <button type="button" className="toolbar-button supertrend-toolbar" onClick={()=>setOpen(true)} title="Supertrend settings: completed candles only">Supertrend {settings.period} × {settings.multiplier} · {result.error ? 'Unavailable' : last ? last.direction===1 ? 'Up' : 'Down' : 'Warming up'}</button>}
