...
export async function recoverBacktest<T extends RunStatus>(runId: string, signal?: AbortSignal, compact = false): Promise<T> {
  ...
  return checkFailure(await api<T>(`${path}/review`, { method: "POST", signal, timeoutMs: 60_000 }));
}
