...
export function HistoricalTradingPage({ mode }: { mode: "backtest" }) {
  ...
  return (
    ...
    >
              ...
              <TradingModeSelectField disabled={batchPreset} label="Level book" help={batchPreset ? "Each run loads its ticker’s date-covered V5 book and advances it causally." : structureBook ? "Loads the preceding closing book and advances it causally during the session. New V5 books grade both support and resistance areas using bounded width and evidence. Chart filters are independent of strategy rules." : "Uses the current v18 structural level contract. Select a validation book to review the new swing levels."}
              ...
              {structureBooks.find((row) => row.id === structureBook)?.version === 'causal-swing-closing-book-5'
                ? <p className="configuration-help">New V5 books select support and resistance areas with evidence score ≥30/100; legacy books retain unscored supports. It does not use p_norm or a prior-close price cutoff. Display score and opacity are configurable in the chart indicator.</p>
